...
class PositionalEncoding(nn.Module):
    ...
    Local replacement for ``speechbrain.lobes.models.transformer.Transformer.PositionalEncoding``
    so that the heavyweight speechbrain package is not required.
    """
...
class DilatedCausalConvEncoder(nn.Module):
    ...
    def forward(self, x, ctx_buf):
        ...
        for i in range(self.num_layers):
            ...
            ctx_buf[..., buf_start_idx:buf_end_idx] = dcc_in[
                ..., -self.buf_lengths[i] :
            ]
...
class CausalTransformerDecoderLayer(torch.nn.TransformerDecoderLayer):
    ...
    def forward(
        self, tgt: Tensor, memory: Optional[Tensor] = None, chunk_size: int = 1
    ) -> Tensor:
...
class CausalTransformerDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.unfold = nn.Unfold(
            kernel_size=(ctx_len + chunk_size, 1), stride=chunk_size
        )
...
class MaskNet(nn.Module):
    def __init__(
        ...
        self.encoder = DilatedCausalConvEncoder(
            channels=model_dim, num_layers=num_enc_layers
        )
...
class Net(nn.Module):
    ...
    def init_buffers(self, batch_size, device):
        ...
        out_buf = torch.zeros(
            batch_size, self.model_dim, self.out_buf_len, device=device
        )
    ...
    def forward(
        self, inputs, init_enc_buf=None, init_dec_buf=None, init_out_buf=None, pad=True
    ):
        """
        ...
        if init_enc_buf is None or init_dec_buf is None or init_out_buf is None:
            assert (
                init_enc_buf is None and init_dec_buf is None and init_out_buf is None
...
def metrics(mixed, output, gt):
    ...
    for m_fn in [snr, si_snr]:
        metrics[m_fn.__name__] = metric_i(m_fn, mixed[:, : gt.shape[1], :], output, gt)
