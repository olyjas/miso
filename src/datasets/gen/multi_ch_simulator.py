...
class BaseSimulator(object):
    ...
    def get_metadata(self):
        ...
        for i, source_id in enumerate(self.source_order):
            source = {
                "position": self.source_positions[i],
                "order": source_id,
    ...
    def initialize_room_with_random_params(
        self,
        num_sources: int,
        duration: float,
        ann_list: list,
...
class CATTRIR_Simulator(BaseSimulator):
    ...
    def get_rirs(self):
        ...
        for i in range(num_sources):
            path = os.path.join(
                self.room_dir, f"CATT_{rt60}_{self.hrtf_indices[i]}.wav"
            )
    ...
    def initialize_room_with_random_params(
        ...
        return super().initialize_room_with_random_params(
            num_sources, duration, ann_list, nbackground_sources
        )
...
class SOFASimulator(BaseSimulator):
    def __init__(self, sofa_text_file, **kwargs) -> None:
        ...
        with open(sofa_text_file, "r") as f:
            ...
            self.sofa_files = [
                os.path.join(sofa_dir, x) for x in self.subject_sofa_list
            ]
    ...
    def initialize_room_with_random_params(
        ...
        mic_positions = self.HRTF.Receiver.Position.get_values(system="cartesian")[
            ..., 0
        ]
    ...
    def get_rirs(self):
        ...
        for i in range(num_sources):
            ...
            if key in self.hrtf_cache:
                ...
                rir = self.HRTF.Data.IR.get_values(indices=self.hrtf_indices[i]).astype(
                    np.float32
                )
...
class CIPIC_Simulator(SOFASimulator):
    def randomize_sources(self, num_sources):
        source_positions = []
        hrtf_indices = []
        random_source_positions = random.sample(
            range(self.HRTF.Dimensions.M), num_sources
        )
        for i in range(num_sources):
            sofa_indices = {"M": random_source_positions[i]}
            pos = self.HRTF.Source.Position.get_values(
                system="cartesian", indices=sofa_indices
            ).tolist()
            source_positions.append(pos)
...
class CIPIC_HRTF_Simulator(CIPIC_Simulator):
    pass
...
class SBSBRIR_Simulator(BRIR48kHz_Simulator):
    def randomize_sources(self, num_sources):
        ...
        random_source_positions = random.sample(
            range(self.HRTF.Dimensions.E), num_sources
        )
        random_measurement_rotation = np.random.randint(self.HRTF.Dimensions.M)
        for i in range(num_sources):
            # sofa_indices = {"M":0, "E":0}
            sofa_indices = {
                "M": random_measurement_rotation,
                "E": random_source_positions[i],
            }
            pos = self.HRTF.Emitter.Position.get_values(
                system="cartesian", indices=sofa_indices
            ).tolist()
            source_positions.append(pos)
            hrtf_indices.append(sofa_indices)
...
class RRBRIR_Simulator(BRIR48kHz_Simulator):
    pass
...
class PRASimulator(object):
    ...
    def initialize_room_with_random_params(self, num_sources: int, duration: float):
        ...
        for i in range(num_sources):
            source_pos = self._get_random_source_pos(
                self.left_wall, self.right_wall, self.bottom_wall, self.top_wall, self.K
            )
        ...
        self.source_order = [
            num_sources - 1
        ] + self.source_order  # Background is always last
    ...
    def simulate(self, source_audio):
        """
        ...
        corners = np.array(
            [
                [self.left_wall, self.bottom_wall],
                [self.right_wall, self.bottom_wall],
                [self.right_wall, self.top_wall],
                [self.left_wall, self.top_wall],
            ]
        ).T

        room = pra.room.Room.from_corners(
            corners, absorption=self.absorption, fs=self.fs, max_order=self.max_order
        )

        mic_array = np.array(
            self.mic_positions
        ).T  # pra.circular_2D_array(center=[0., 0.], M=self.M, phi0=180, radius=self.mic_distance * 0.5 * 1e-2)
