...
def is_valid_background(label_id, foreground_ids):
    ...
    excluded_subtrees = [my_ontology.MUSIC, my_ontology.get_id_from_name("Human voice")]
    for subtree in excluded_subtrees:
        if my_ontology.is_reachable(subtree, label_id):
            return False

    for fg_id in foreground_ids:
        if my_ontology.is_reachable(label_id, fg_id) or my_ontology.is_reachable(
            fg_id, label_id
        ):
            return False
...
def write_scaper_source(
    ...
    for index, sample_data in tqdm(dataset.iterrows(), total=dataset.shape[0]):
        ...
        fname = os.path.join(
            dataset_name.lower() + "_" + os.path.basename(sample_data["fname"])
        )
...
def main(
    ...
    id2classname = {}
    class_data = read_yaml(class_definitions)
    ...
    df.to_csv(os.path.join(datasets_dir, "start_times.csv"))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--datasets_dir",
        type=str,
        default="data/BinauralCuratedDataset",
        help="Path to directory containing all datasets.",
    )
    parser.add_argument(
        "--class_definitions",
        type=str,
        default="data/Classes.yaml",
    ...
    parser.add_argument(
        "--fg_output_dir",
        type=str,
        default="data/BinauralCuratedDataset/scaper_fmt",
        help="Path to directory to write scaper formatted data.",
    )
    parser.add_argument(
        "--bg_output_dir",
        type=str,
        default="data/BinauralCuratedDataset/bg_scaper_fmt",
        help="Path to directory to write scaper formatted data.",
    )
    parser.add_argument(
