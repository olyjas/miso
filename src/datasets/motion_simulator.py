...
class MotionSimulator:
    def __init__(self, sr: int, frame_duration: float = 0.025) -> None:
        ...
        self._call(
            "simulator_init",
            ctypes.pointer(self.simulator_p),
            self.sr,
            self.frame_duration,
            init=True,
        )
    ...
    def add_source(self, data: np.ndarray, path: np.ndarray):
        ...
        expected_number_of_frames = int(
            np.ceil(expected_time / self.frame_duration.value)
        )
    ...
    def simulate(self):
        ...
        self._call(
            "simulator_simulate",
            ctypes.pointer(data_p),
            ctypes.pointer(num_sources),
            ctypes.pointer(num_samples),
        )
        ...
        audio = np.array(data_p[:num_floats], dtype=np.float32).reshape(
            num_sources, num_samples // 2, 2
        )
...
class CIPICMotionSimulator2(CIPICSimulator):
    def __init__(
        self, sofa_text_file, sr, frame_duration=0.025, use_piecewise_arcs=False
    ) -> None:
        ...
        self.MIN_SPEED = np.pi / 36  # 10 dps
        self.MAX_SPEED = 2 * np.pi  # 360 dps
    ...
    def get_piecewise_arc_path(self, rng: random.Random, t: np.ndarray, debug=False):
        ...
        while i < len(t):
            ...
            if p < bernoulli_p:
                ...
                wtheta = rng.uniform(self.MIN_SPEED, self.MAX_SPEED) * (
                    (-1) ** rng.randint(0, 1)
                )

                # Choose speed for phi (positive or negative)
                wphi = rng.uniform(self.MIN_SPEED, self.MAX_SPEED) * (
                    (-1) ** rng.randint(0, 1)
                )
    ...
    def get_face_to_face_source_path(self, seed: int, t: np.ndarray):
        ...
        theta_phi = rng.multivariate_normal(
            mean=[0, np.pi / 2], cov=[[np.pi / 30, 0], [0, np.pi / 30]], size=len(t)
        )  # [*, 2]
        theta_phi[:, 0] = np.clip(theta_phi[:, 0], -np.pi / 2, np.pi / 2)
        theta_phi[:, 1] = np.clip(theta_phi[:, 1], 0, np.pi)
    ...
    def simulate(self, srcs, noise, seed=None, face_to_face_idx=None, debug=False):
        """
        ...
        for i, src in enumerate(srcs):
            if face_to_face_idx is not None and i == face_to_face_idx:
                path = self.get_face_to_face_source_path(seed, t)
                # _plot_coordinates(path, f'Enroll')
            else:
                path = self.get_random_source_path(rng, t)
                # _plot_coordinates(path, f'Source {i}')

            self._simulator.add_source(src, path)
            path_srcs.append(path)

        noise_path = self.get_random_source_path(rng, t)
...
class RRBRIRMotionSimulator(CIPICMotionSimulator2):
    ...
    def get_face_to_face_source_path(self, seed: int, t: np.ndarray):
        ...
        r = 1.5  # m
        theta_phi = rng.multivariate_normal(
            mean=[0, np.pi / 2], cov=[[np.pi / 30, 0], [0, 0]], size=len(t)
        )  # [*, 2]
        theta_phi[:, 0] = np.clip(theta_phi[:, 0], -np.pi / 2, np.pi / 2)
...
def test_simulator():
    ...
    T = 5
    fs = 8000
    sim = CIPICMotionSimulator2(
        "data/360-BRIR-FOAIR-database/Binaural/SOFA/val.txt", fs
    )
    y1 = read_with_pad("tests/speech_samples/speech1.wav", fs)
    y2 = read_with_pad("tests/speech_samples/speech2.wav", fs)
    y3 = read_with_pad("tests/speech_samples/speech3.wav", fs)

    src, noise, path_src, noise_path = sim.simulate(
        np.array([y1, y2]), np.array([y3]), seed=3, debug=True
    )
...
def test_path_generation():
    ...
    rng = random.Random(2)
    t = np.arange(0, 5, dt)
    path, theta, phi, prob, max_prob = simulator.get_piecewise_arc_path(
        rng, t, debug=True
    )
