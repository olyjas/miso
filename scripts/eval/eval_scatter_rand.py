...
Same as eval_scatter.py, except it evaluates a RANDOM sample of N val indices
(seeded for reproducibility) instead of the first N by index — useful for
checking whether a fixed-order gap against Lightning's own logged val metrics
is a sampling artifact or something else.
...
divergent cases (high SI-SDRi but low plain SNRi — i.e. good scale-invariant
separation but a poor absolute-volume match), saves audio clips for them,
and checks whether the mixture SNR baseline was already high for those cases.

Usage:
    python eval_scatter_rand.py \
...
def main():
    ...
    p.add_argument("--seed",        type=int, default=42,
                   help="Seed for random index sampling")
    ...
    indices = random.Random(args.seed).sample(range(len(val_ds)), n)
    print(f"Val set: {len(val_ds)} samples  |  evaluating {n} (random, seed={args.seed})")
    ...
    for i, idx in enumerate(indices):
        ...
        rows.append(dict(
            ...
            divergence=round(si_sdri - snri, 4),
        ))
        if (i + 1) % 20 == 0:
            print(f"  [{i+1:3d}/{n}]  SI-SDRi={si_sdri:+6.2f}  SNRi={snri:+6.2f}  div={si_sdri-snri:+5.2f}")
    ...
    if HAS_MPL:
        ...
        ax.scatter(si_snri_arr, si_sdri_arr, alpha=0.45, s=20, color="#4C72B0", zorder=3,
                   label=f"n={n} val samples (random)")
        ...
        ax.set_title(f"{args.trigger} — SI-SDRi vs SI-SNRi  (n={n}, random)", fontsize=13)
        ...
        fig2.suptitle(f"{args.trigger} — metric distributions  (n={n}, random)", fontsize=13)
        fig2.tight_layout()
    ...
    # ── Most divergent: high SI-SDRi, low plain SNRi ─────────────────────────
    divergent = sorted(rows, key=lambda r: r["divergence"], reverse=True)[: args.save_clips]
    print(f"\nTop {args.save_clips} divergent samples (SI-SDRi high, SNRi low):")
    print(f"  {'idx':>4}  {'SI-SDRi':>8}  {'SNRi':>8}  {'diff':>7}  {'mix_SNR':>8}")
    for r in divergent:
        print(f"  {r['idx']:4d}  {r['si_sdri']:+8.2f}  {r['snri']:+8.2f}  "
    ...
    for r in divergent:
        ...
        tag = f"idx{idx:03d}_sisdri{r['si_sdri']:+.1f}_snri{r['snri']:+.1f}"
